...
class ShareService {
  ...
  void initializeShareListener(void Function(ShareData) onShareReceived) {
    ...
    _intentDataStreamSubscription = 
      ...
      if (text != null && text.isNotEmpty && !_isProcessing) {
        _processShareData(ShareData(text: text, type: ShareType.text));
      }
    }, onError: (err) {
      print('Error receiving text: $err');
    });

    _intentDataStreamSubscription = 
        ReceiveSharingIntent.getMediaStream().listen((List<SharedMediaFile>? mediaList) {
      if (mediaList != null && mediaList.isNotEmpty && !_isProcessing) {
    ...
    ReceiveSharingIntent.getInitialText().then((String? text) {
      if (text != null && text.isNotEmpty && !_isProcessing) {
        _pendingShareData = ShareData(text: text, type: ShareType.text);
      }
    });

    ReceiveSharingIntent.getInitialMedia().then((List<SharedMediaFile>? mediaList) {
      if (mediaList != null && mediaList.isNotEmpty && !_isProcessing) {
        _processMediaFiles(mediaList);
      }
    });
